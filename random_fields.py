...
class GaussianRF1d(object):
    ...
    def __init__(self, s, L=2*math.pi, alpha=2.0, tau=3.0, sigma=None, mean=None, boundary="periodic", device=None, dtype=torch.float64):
        ...
        self.sqrt_eig = s*self.sigma*norm_const*((const*k**2 + tau**2)**(-alpha/2.0))
        self.sqrt_eig[0] = 0.0
        self.sqrt_eig[s//2:] = 0.0

    def sample(self, N, xi=None):
        if xi is None:
            xi  = torch.randn(N, self.s, 2, dtype=self.dtype, device=self.device)
        
        xi[...,0] = self.sqrt_eig*xi [...,0]
        xi[...,1] = self.sqrt_eig*xi [...,1]
        
        u = fft.ifft(torch.view_as_complex(xi), n=self.s).imag
